...
The study area and its tier definitions were suggested by local resident and Seafield ward community councillor Ian MacLeod, based on local knowledge of how the town centre functions in practice.
...
```{r moran-scatter, fig.height=6, fig.cap="Moran scatterplot for deprivation (inverted SIMD rank, so high values = more deprived). Each point is a datazone; the x-axis shows its deprivation score, the y-axis shows the average score of its neighbours. Red points are datazones in the wider Ayr study area. The positive slope confirms that deprivation clusters spatially --- deprived areas tend to be surrounded by other deprived areas."}
plot_moran_scatterplot(sa, "SIMD2020v2_Rank", weights, highlight_dzs = all_focus_dzs, invert = TRUE)
```
...
```{r lisa, fig.height=7, fig.cap="LISA cluster map based on deprivation score (inverted SIMD rank, so high = more deprived). High-High clusters (red) indicate statistically significant concentrations of deprivation. Low-Low clusters (blue) indicate affluence clusters. Map zoomed to the Wider Ayr study area."}
sa <- run_local_morans_i(sa, "SIMD2020v2_Rank", weights, invert = TRUE)
